...
* 파이썬에서는 서브 클래스의 관계가 삼단논법을 따를 필요가 없습니다. 아무나 자유롭게 메인 클래스에  자신만의 `__subclasscheck__`를 만들 수 있습니다. 
* `issubclass(cls, Hashable)`이 호출되면, `cls` 안에 거짓이 아닌 "`__hash__`" 메소드나 상속된 것을 찾습니다.
